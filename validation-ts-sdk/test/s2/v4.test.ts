...
describe('2_STORY', () => {
  ...
  describe('Layer3 Full-Node Build (252)', () => {
    ...
    it('4.3 block 동기화가 정상적으로 이루어져있어야 한다.', async () => {
      ...
      console.log(
        `4.3 block 동기화가 정상적으로 이루어져있어야 한다. L3 Feed Port 접속 확인. \n  예상 결과 - client connect {host}:{port}: true \n  실제 결과 - client connect ${host}:${port}: ${ansi.Green}${result}${ansi.reset}`,
      );
      expect(result).toEqual(true);
    });

    it('4.4 (full node에서) 발행된 transaction이 sequencer로 즉시 전달되어야 한다.', async () => {
    ...
    it('4.5 ~ 8 command', async () => {
      ...
      console.log(
        `4.5 - [l3node]    ./minimal-node.bash script send-l3 --ethamount 0.1 --wait --l3url ws:/${l3host}:${l3port} \n      [full-node] ./minimal-node.bash script send-l3 --ethamount 0.1 --wait --l3url ws:/${l3fnhost}:${l3fnport} \n4.6 - eth.getTransaction \n4.7 - eth.getBlock \n4.8 - ./minimal-node.bash docker stop l3node_full_node \n      ./minimal-node.bash --run --detach --l3-node-sp`,
      );
    });
