...
describe('4_STORY', () => {
  ...
  describe('Verify L3 Batch Transaction', () => {
    it('1.1', async () => {
      ...
      let from = 0;
      let isStop = false;
      let batchCount = 5; // Latest Batch Data 15개를 조회합니다.
      ...
      console.log(`\n\nCurrent L2 Safed Block Number : ${l2SafedBlock}`);
      console.log(`Total Batch Data                  : ${batchTransactions.length} \n`);
      batchTransactions.map(batch => {
        const { blockNumber, args } = batch;
        const batchNumber = args.batchSequenceNumber;
        console.log(
          `Batch Number : ${batchNumber} ||  Batch Tx Block Number : ${blockNumber} || Statue : ${
            l2SafedBlock >= blockNumber ? 'Finalized' : 'Unfinalized'
          }\n`,
        );
        return { blockNumber, batchNumber };
      });
    });
