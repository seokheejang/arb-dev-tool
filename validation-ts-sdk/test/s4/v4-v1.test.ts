...
describe('4_STORY', () => {
  ...
  beforeAll(async () => {
    ...
    l2_2_w = new CustomWallet(ENV.L3_TEST_2_KEY, l2_seq);
  ...
  describe('ETH 기반 Validator Library 개발', () => {
    it('1.1', async () => {
      console.log('validator1 stake 출금');
        ...
        try {
          ...
          rollupContract.on(
            ...
            async (user: string, initialBalance: string, finalBalance: string) => {
              ...
              await sleep(5000);
              console.log('execWithdraw', execWithdraw);
              if (execWithdraw == 1) {
                const v1WalletContactBal = await l2_seq.getBalance(v1scwAddr);
                try {
                  console.log(`[${getTime()}] Validator Wallet - withdrawEth Tx 발생`);
                  const withdrawTx = await v1scw.withdrawETHFromWalletContract(
      ...
      try {
        ...
        const unstakeTx = await v1scw.unstake(v1scwAddr, rollupCA);
